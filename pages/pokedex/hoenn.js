...
export default function Home({poke}) {
  const [randomId, setRandomId] = useState(252);

    useEffect(() => {
        setRandomId(Math.floor(Math.random() * (386 - 252 + 1)) + 252);
    }, []);
...
export async function getStaticProps(context) {
  try {
    ...
    const generatePokemonPromises = () => Array(135).fill().map((_, index) =>
      fetch(getPokemonUrl(index + 252)).then(response => response.json()))
